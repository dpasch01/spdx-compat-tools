...
public class ViolationAnalysisInfo {
	...
	public ViolationAnalysisInfo(SpdxLicensePairConflictError... unaryErrors) {
		...
		System.out
				.println(
						"Conjunctive: " + Arrays
								.asList(getMultipleDeclaredLicenses(conjunctiveBucket
										.toArray(new SpdxLicensePairConflictError[conjunctiveBucket.size()])))
						.toString());
		System.out
				.println(
						"Disjunctive: " + Arrays
								.asList(getMultipleDeclaredLicenses(disjunctiveBucket
										.toArray(new SpdxLicensePairConflictError[disjunctiveBucket.size()])))
						.toString());
		System.out.println();
		...
		if (disjunctiveBucket.isEmpty()) {
			...
			if (LicenseCompatibility.areCompatible(declared)) {
				...
				proposedLicenses.addAll(LicenseCompatibility.proposeLicense(declared));
			...
			while (visitedBucket
				...
				if (LicenseCompatibility.areCompatible(traverseBucket.toArray(new String[traverseBucket.size()]))) {
					this.setViolationDetected(false);
					break;
				} else {
					this.setViolationDetected(true);
					proposedLicenses.addAll(LicenseCompatibility.proposeLicense(traverseBucket.toArray(new String[traverseBucket.size()])));
					if (proposedLicenses.isEmpty()) {
						this.setAdjustFeasible(false);
					} else {
						this.setAdjustFeasible(true);
